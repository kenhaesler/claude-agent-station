<script lang="ts">
  ...
  const columns: Column[] = [
    ...
    { label: 'Done', color: 'var(--color-emerald)', borderColor: 'var(--color-emerald)', states: ['approved', 'completed'] },
    { label: 'Blocked', color: 'var(--color-rose)', borderColor: 'var(--color-rose)', states: ['rejected', 'escalated', 'paused', 'failed', 'cancelled'] },
  ];
  ...
  function getPriorityColor(priority: number): string {
    if (priority >= 4) return 'var(--color-rose)';      // critical
    if (priority >= 3) return 'var(--color-amber)';      // high
    if (priority >= 2) return 'var(--color-indigo)';     // medium
    return 'var(--color-tertiary)';                       // low
  }
...
<div class="flex gap-3 overflow-x-auto no-scrollbar">
  {#each columns as col}
    ...
    <div class="flex-1 min-w-[160px] flex flex-col">
      ...
      >
        <span class="text-xs font-semibold text-secondary uppercase tracking-wider">{col.label}</span>
        <span
          class="badge text-[9px] py-0 px-1.5"
      ...
      <div class="flex flex-col gap-1.5">
        {#if colItems.length === 0}
          <div class="text-[10px] text-ghost font-mono text-center py-3">--</div>
        {:else}
