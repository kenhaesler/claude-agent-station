    ...
    {#each projects as project (project.id)}
      <div class="glass rounded-lg p-4 hover:bg-surface-2/30 transition-colors">
        <div class="flex items-center justify-between mb-2">
          <button onclick={() => navigate(`/projects/${project.id}`)} class="text-sm font-medium text-text hover:text-info transition-colors truncate text-left">
            {project.repo}
          </button>
          <button
        ...
        {#if !project.enabled}
          <div class="mt-1 text-[10px] text-text-muted opacity-50">Disabled</div>
        {/if}
      </div>
    {/each}
  </div>
...
<Modal show={showCreateModal} onClose={() => showCreateModal = false} title="Add Project">
  <div class="space-y-3">
    ...
    <div class="flex gap-3">
      <select bind:value={newMode} class="flex-1 px-3 py-2 rounded-lg bg-bg text-text text-sm border border-border focus:border-focus outline-none">
        <option value="full">Full</option>
        <option value="analyze">Analyze</option>
        <option value="plan">Plan</option>
        <option value="plan_only">Plan Only</option>
      </select>
      <select bind:value={newPriority} class="flex-1 px-3 py-2 rounded-lg bg-bg text-text text-sm border border-border focus:border-focus outline-none">
